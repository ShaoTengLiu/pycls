...
from pycls.models.sa.modules import Subtraction, Subtraction2, Aggregation
...
class SAN(nn.Module):
    def __init__(self, block=Bottleneck):
        ...
        sa_type = cfg.SAN.SA_TYPE
        layers = cfg.SAN.LAYERS
        kernels = cfg.SAN.KERNELS
        num_classes = cfg.MODEL.NUM_CLASSES

        c = int(64 * cfg.MODEL.WIDTH)
        self.conv_in, self.bn_in = conv1x1(3, c), get_norm(cfg.SAN.NORM_FUNC, c)
        self.conv0, self.bn0 = conv1x1(c, c), get_norm(cfg.SAN.NORM_FUNC, c)
        self.layer0 = self._make_layer(sa_type, block, c, layers[0], kernels[0])

        c *= 4
        self.conv1, self.bn1 = conv1x1(c // 4, c), get_norm(cfg.SAN.NORM_FUNC, c)
        self.layer1 = self._make_layer(sa_type, block, c, layers[1], kernels[1])

        c *= 2
        self.conv2, self.bn2 = conv1x1(c // 2, c), get_norm(cfg.SAN.NORM_FUNC, c)
        self.layer2 = self._make_layer(sa_type, block, c, layers[2], kernels[2])

        c *= 2
        self.conv3, self.bn3 = conv1x1(c // 2, c), get_norm(cfg.SAN.NORM_FUNC, c)
        self.layer3 = self._make_layer(sa_type, block, c, layers[3], kernels[3])

        c *= 2
        self.conv4, self.bn4 = conv1x1(c // 2, c), get_norm(cfg.SAN.NORM_FUNC, c)
